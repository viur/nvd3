/* nvd3 version 1.8.3 (https://github.com/novus/nvd3) 2018-11-30 */
(function(){
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            data.forEach(function (series, index) {
                series.values
                    .sort(function (d1, d2) {
                        return getY(d2) - getY(d1);
                    })
                    .forEach(function (point, i) {

                        if (i > 0) {
                            point.percent = (getY(point) * 100) / getY(series.values[i - 1]);
                        }
            ...
            barsEnter.append('rect')
                .attr('height', 0)
                .attr('width', x.rangeBand() * .25 / data.length);
            ...
            bars
                ...
                .attr('class', rectClass)
                .watchTransition(renderWatch, 'discreteBar: bars rect')
                .attr('width', x.rangeBand() * .5 / data.length);
            bars.watchTransition(renderWatch, 'discreteBar: bars')
                //.delay(function(d,i) { return i * 1200 / data[0].values.length })
                .attr('transform', function (d, i) {
...
nv.models.funnelChart = function () {
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            defsEnter.append('clipPath')
                .attr('id', 'nv-x-label-clip-' + funnel.id())
                .append('rect');

            g.select('#nv-x-label-clip-' + funnel.id() + ' rect')
                .attr('width', x.rangeBand() * (staggerLabels ? 2 : 1))
                .attr('height', 16)
                .attr('x', -x.rangeBand() / (staggerLabels ? 1 : 2 ));
    ...
    funnel.dispatch.on('elementMouseover.tooltip', function (evt) {
        evt['series'] = {
            key: chart.x()(evt.data),
            value: chart.y()(evt.data),
            color: evt.color
...
nv.models.gaugeChart = function () {
    ...
    var margin = {top: 30, right: 20, bottom: 20, left: 20}
        ...
        , noData = null
        , duration = 250
        , dispatch = d3.dispatch('tooltipShow', 'tooltipHide', 'stateChange', 'changeState', 'renderEnd', 'pointClick')
        ;
    ...
    function chart(selection) {
        ...
        selection.each(function (data) {
            ...
            pie.pieStartAngle(-Math.PI + 0.4);
            pie.pieEndAngle(Math.PI - 0.4);

            pie.value(data[0].value);
            pie.titleOffset(-15);
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            if (data && data.length && slices && slices < data.length) {
                var total = data.slice(slices-1).reduce(function(sum, d) {
                    return sum + d.value;
                }, 0);
                data = data.slice(0, slices-1);
                data.push({label: 'Others', value: total});
            }
...
nv.models.radar = function () {
    ...
    function chart(selection) {
        selection.each(function (data) {
            ...
            wrap.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
            g.select('.nv-groups').attr('transform', 'translate(' + availableWidth / 2 + ',' + availableHeight / 1.5 + ')');
            g.select('.nv-scatterWrap').attr('transform', 'translate(' + availableWidth / 2 + ',' + availableHeight / 1.5 + ')');
            g.select('.nv-gridWrap').attr('transform', 'translate(' + availableWidth / 2 + ',' + availableHeight / 1.5 + ')');
            ...
            var ax = wrap.select("g.axes").selectAll("g.nv-axis").data(legs);
            ax.exit().remove();
...
nv.models.stackedBar = function() {
    ...
    var margin = {top: 0, right: 0, bottom: 0, left: 0}
        ...
        , container = null
        , getX = function(d) { return d.x }
        , getY = function(d) { if(d.display.y) return d.display.y; return d.y }
        , forceY = [0] // 0 is forced by default.. this makes sense for the majority of bar graphs... user can always do chart.forceY([]) to remove
        , clipEdge = true
        , stacked = false
        , stackOffset = 'expand' // options include 'silhouette', 'wiggle', 'expand', 'zero', or a custom function
        , color = nv.utils.defaultColor()
        , hideable = false
        , barColor = null // adding the ability to set the color for each rather than the whole group
        , disabled // used in conjunction with barColor to communicate from multiBarHorizontalChart what series are disabled
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            if (stacked) {
                ...
                parsed.forEach(function(series, i){
                    ...
                    if (series.nonStackable) {
                        ...
                        if (i > 0 && parsed[i - 1].nonStackable){
                            parsed[i].values.map(function(d,j){
                                d.y0 -= parsed[i - 1].values[j].display.y;
                                d.y1 = d.y0 + d.display.y;
                            });
                        }
                    }
                });
            ...
            var exitTransition = renderWatch
                .transition(groups.exit().selectAll('rect.nv-bar'), 'stackedbarExit', Math.min(100, duration))
                .attr('y', function(d, i, j) {
                    var yVal = y0(0) || 0;
                    if (stacked) {
                        if (data[d.series] && !data[d.series].nonStackable) {
            ...
            var barSelection =
                bars.watchTransition(renderWatch, 'stackedbar', Math.min(250, duration))
                    .delay(function(d,i) {
                        return i * duration / data[0].values.length;
                    });
            if (stacked){
                barSelection
                    ...
                    .attr('height', function(d,i,j) {
                        if (!data[j].nonStackable) {
                            return Math.max(Math.abs(y(d.display.y+d.y0) - y(d.y0)), 0);
                        } else {
                            return Math.max(Math.abs(y(getY(d,i)) - y(0)), 0) || 0;
                        }
                    })
        ...
        renderWatch.renderEnd('stackedbar immediate');
...
nv.models.stackedBarChart = function() {
    "use strict";
    ...
    var multibar = nv.models.stackedBar()
        , xAxis = nv.models.axis()
        , yAxis = nv.models.axis()
        , interactiveLayer = nv.interactiveGuideline()
        , legend = nv.models.legend()
    ...
    state.stacked = true; // DEPRECATED Maintained for backward compatibility
    ...
    function chart(selection) {
        ...
        renderWatch.renderEnd('stackedbarchart immediate');
        return chart;
    }
